...
class Prices(Extract):
    ...
    def get_data(self):
        ...
        yf.download(self.tickers, start = self.starting_date, end = self.ending_date, group_by = 'ticker', session=self.session).stack(level=0).to_parquet(self.output_path)
...
class Earnings_Estimate(Extract):
    ...
    def get_data_for_ticker(self, ticker, progress=True):
        df = pd.DataFrame(yf.Ticker(ticker, session=self.session).get_earnings_estimate())
...
class Earnings_History(Extract):
    ...
    @multitasking.task
    def get_data_for_ticker(self, ticker, progress=True):
        df = pd.DataFrame(yf.Ticker(ticker, session=self.session).get_earnings_history())
        df['Ticker'] = ticker  # add a column for ticker name
        self.results.append(df)
